...
from crud.patient_crud import get_patient_profile
...
@router.get("/{line_user_id}/nutrition-targets", response_model=NutritionTargetRead)
async def get_nutrition_targets(
    ...
    if not profile:
        # If no profile, return default targets
        from model.models import PatientProfile
        dummy_profile = PatientProfile(
            user_id=user_id, first_name="", last_name="", age=0, gender="", phone="",
            height=0, weight=0, blood_pressure="", smoking="", alcohol=""
        )
        return calculate_nutrition_targets(dummy_profile)
